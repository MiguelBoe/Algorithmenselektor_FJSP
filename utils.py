    ...
    for job in range(job_count):
        ...
        for task in range(task_count):
            machine_id = random.choice(
                list(set(range(task_count)).difference(set(machine_id_del)))
            )
...
jobs_data = job_generator(max_job_count, max_job_count, max_processing_time)
print(jobs_data)
...
def visualize_schedule(assigned_jobs, all_machines, plan_date):
    ...
    for machine in all_machines:
        ...
        for assigned_task in assigned_jobs[machine]:
            ...
            temp = dict(
                Task=machine,
                Start=plan_date + assigned_task.start,
