...
def solution(status, solver):
    ...
    if status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
        _extracted_from_solution_8(assigned_task_type, solver)
    else:
...
# TODO Rename this here and in `solution`
def _extracted_from_solution_8(assigned_task_type, solver):
    print("Solution:")
    ...
    for machine in range(num_machines):
        ...
        for assigned_task in assigned_jobs[machine]:
            ...
            # Add spaces to output to align columns.
            sol_line += "%-15s" % ("[%i,%i]" % (start, start + duration))
