...
# vielleicht zusammenfassung SPT/LPT?
...
def get_prio_task_RPT(task_on_machine: list[Task], jobs_data: JobList, priority_rule: str) -> Task:

    # JobIDs und TaskIDs der Task raussuchen, zwischen den entschieden werden muss
    job_ids = [task.job_id for task in task_on_machine]
    task_ids = [task.task_id for task in task_on_machine]

    # Berechnen der Restlichen Bearbeitungszeiten der Jobs
    remaining_processing_time = {}
    for index, (job_id, task_id) in enumerate(zip(job_ids, task_ids)):
        remaining_processing_time |= {
            index: sum(task[1] for task in jobs_data.list_of_jobs[job_id][task_id:]) for job in job_ids
        }
...
def get_predecessor(schedule, task_id, task_on_machine_idx, machine_id, job_id):
    return [
        ...
        if (v.job_id == job_id and v.task_id == task_id - 1)
        or (v.machine_id == machine_id and v.task_on_machine_idx == task_on_machine_idx - 1)
    ]
...
if __name__ == "__main__":
    ...
    schedule = giffler_thompson(jobs_data, "LRPT")
